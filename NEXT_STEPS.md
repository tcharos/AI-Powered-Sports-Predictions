...
- [ ] **D0 — Fix the dead draw model.** `train_model.py:train_draw` produces a model with **0% recall on draws** — it collapsed to majority class ("not draw"), achieves 74.6% accuracy purely by virtue of draws being ~25% of matches. Has been a no-op in the 2-stage 1X2 prediction for an unknown amount of time; the multi-class 1X2 model has been carrying the entire draw signal alone. Surfaced in the 2026-05-19 retrain log (`Draw Model | Acc: 0.7461 | Recall (Draws): 0.0000 | Precision: 0.0000`) but is pre-existing. Likely fix: set `scale_pos_weight = (1 - p_draw) / p_draw ≈ 3.0` on the binary classifier, possibly combined with a lower decision threshold (don't require P(draw) > 0.5 to predict draw — try 0.3 or 0.35). Verify post-fix: recall > 0.20 on the test set. ~1 hour of work. **Highest-priority D-series item** because every D1/D2 gain compounds with a working draw model.
