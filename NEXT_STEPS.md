...
**Current verdict (2026-05-26): D3 deferred — consistency motivation refuted by the check.** Its remaining motivations are weaker / not currently needed: interpretable per-league α/β strengths (nice, not an edge), BTTS / correct-score markets (we don't bet them), dropping the Platt calibration layer (C4 already works). Revisit only if we decide to bet score-derived markets, or if a future retrain surfaces real 1X2↔O/U contradictions (re-run the check). The DC scoreline + check tooling is kept under `ml_project/dixon_coles/` for that re-check.
