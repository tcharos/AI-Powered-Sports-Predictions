...
**v1 build plan (post-reframe + probe)** — three steps, each isolated under `scripts/d4_injuries/` / `output/d4_probe/` until proven, touching no production flow until the adjuster ships behind a flag:
...
2. **Injury adjuster** (post-model, capped). New `apply_availability_adjustment(probs, home_avail, away_avail)` modelled on `_apply_red_card_modifier`: weight = Σ over absences of `reason_weight × importance_weight`, shift 1X2 away from the more-depleted side toward opponent + draw, magnitude ∝ weight-differential, capped at a `MAX_AVAIL_SHIFT`. v1 `importance_weight = 1` for everyone (importance-blind — honest limitation); `reason_weight`: injury/suspension = 1, inactive ≈ 0.2, doubtful ≈ 0.5. O/U effect deliberately omitted in v1 (state-dependent, like the red-card O/U decision). Wire into `predict_matches.py` between Platt calibration and the heuristic adjuster, behind a per-sport `use_availability_adjustment` flag (default off) in `betting_config.json`.
3. **Forward validation + calibration**. Log each match's pre/post-adjustment probs + the absence list to a JSONL; once enough settle, compare adjusted vs unadjusted Brier on real outcomes (forward-only — see caveat above). Tune `MAX_AVAIL_SHIFT` + reason weights on that, and gate magnitude *down* in well-covered leagues where the absence is already priced.

**Importance — the one real gap, deferred to v2.** v1 is importance-blind (every absence counts equally), which the D2 lesson says is weak. Two enrichment paths, both keyed off the player_id/name the extractor already captures:
- **Preview-text parse** (free, no API): the summary page's "FLASHSCORE PREVIEW" prose names the *key* absences — a light keyword/proximity parse ("without", "miss", "ruled out" near a player name) upgrades those players' `importance_weight`. Noisy but zero-cost.
- **api-sports/injuries (100 free calls/day) — good fit as enrichment.** A night-before slate filtered to `target_leagues.json` is ~5–15 fixtures, well under 100 calls/day. Use it for player minutes/position → importance weight, and as a cross-check/fallback where Flashscore is thin. *Verify before relying:* (a) coverage of our **less-covered** target leagues (top-5 are priced; the edge is in the long tail), (b) that it exposes a usable importance signal (minutes/starts), (c) recent-past queryability for the retrospective validation backfill.

**NewsAPI — deferred (pushed back).** News sentiment is the *most* market-priced signal (the line moves on news fastest of all — D2 lesson at its strongest), it's noisy, and it needs an NLP pipeline to become a probability shift. High effort, low marginal edge, hard to validate. The Flashscore preview text already covers the "named key absence" need for free. Revisit only if the injury adjuster proves out and importance is still the bottleneck.

**Other sources, for the record** (not needed for v1, assess only if scaling importance): Transfermarkt (richest — injuries+suspensions+market value+history+player→club, but ToS-hostile scraping); Sportmonks/other paid APIs (verify historical injury depth + license); FBref/StatsBomb (great minutes data for weighting, **no injury feed**).
