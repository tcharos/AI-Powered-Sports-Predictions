...
- [x] **C1 — Diagnose per-league miscalibration.** `ml_project/calibration/diagnose.py` + `scripts/run_diagnose_calibration.py`. Uses OOF predictions from a 5-fold TimeSeriesSplit (same fold structure as `train_model.py`) — no leakage. Outputs CSV + Markdown to `output/calibration/diagnose_<ts>.{csv,md}`. **First run (2026-05-19)**: 20 leagues passed N ≥ 100. Headline findings: **(a)** Bundesliga (D1) is the most-biased O/U market — model overestimates Under by +10.9pp (says 50%, actual 39%), inflating Over bets' EV systematically; **(b)** Italian leagues (I1, I2) are most miscalibrated on 1X2; **(c)** the original Lahti vs VPS problem (Finnish league) is **not** in this diagnostic — Finland isn't in `data_sets/MatchHistory/`, so C2 calibration won't directly fix thin-data leagues. The `ev_cap_value` guard from step C0 stays in place as the safety net for that case until we either pool minor leagues into a fallback tier or import Finnish historical data. |
