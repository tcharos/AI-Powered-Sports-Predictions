...
- **Track `data_sets/team_mappings.json` in git.** It's a hand-curated source of truth for Flashscore↔football-data.co.uk name reconciliation, but currently swept up by the broad `data_sets/` line in `.gitignore` (verified `git check-ignore` → `.gitignore:2:data_sets/`). Regenerating it on a fresh clone is non-trivial. Fix: add an explicit negation to `.gitignore` (`!data_sets/team_mappings.json`) so the file falls through the directory-wide rule, then `git add data_sets/team_mappings.json` and commit. Verify `entity_resolver.py` still resolves on a fresh clone after the commit lands.
- **Live-analysis panel shows a CASHED_OUT match after every bet on it is cashed out.** The Live panel's bet-attachment logic (`_attach_open_bets` in `web_ui/app.py:222`) filters by per-bet `status == 'OPEN'`; that part is correct. The leftover row is the *match row itself* — when every bet on a fixture is in a terminal status (CASHED_OUT / WON / LOST / VOID), the live row should drop off the panel unless there's something else still actionable on it. Fix: when iterating live matches, suppress any match whose attached bets list is empty AND whose status is terminal across the board. Confirm against the Machida vs Urawa fixture (cashed out 2026-05-22) — it should not appear in `/football/dashboard`'s Live section.
- **Remove `MAX_CASHOUT_EUR` cap from the cashout-commit path.** The cap was a belt-and-braces guard while validating the two-click flow (scenario #2 in `real_betting/test_case_scenarios.md`). Going forward, the cashout decision must be driven by **live match statistics + the scraped bookmaker offer** (scenarios #3 / #4) — not by a fixed euro ceiling. A €5 cashout on a €2 stake that the live adjuster flags as `stop_loss` should fire regardless of any pre-set cap. Drop `MAX_CASHOUT_EUR` from `real_betting/dryrun_cashout_discovery.py` and from any future bulk-cashout caller; replace with a decision-engine check (recommendation == `CASH_NOW` AND offer parseable AND offer > 0). The `EXECUTE_CASHOUT` opt-in flag stays as the kill switch.
