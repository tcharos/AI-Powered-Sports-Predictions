...
**v1 build plan (post-reframe + probes)** — isolated under `scripts/d4_injuries/` / `output/d4_probe/` until proven, no production touch until the adjuster ships behind a flag. **Step 0 gates everything** — don't build 1–4 until it passes:
0. **Confirm api-sports importance coverage** (the make-or-break above). Verify per-player minutes/rating for the *less-covered* target leagues + name-join feasibility. If it fails, D4 stalls here — reassess fallbacks (Flashscore player pages / Transfermarkt) before committing.
1. **Availability extractor** (Flashscore lineups page). Generalise the probe into a clean reader: per fixture → `{home: [{name, player_id, reason, reason_class}], away: [...]}`, `reason_class ∈ {injury, suspension, inactive, doubtful, other}`. Reuses the spider's Playwright infra; the lineups URL is derivable from the `mid` we already capture during prediction scraping. Output `output/d4_probe/availability_<date>.json`. (This part is fully de-risked by the probe — can build independently of step 0, it just isn't *useful* without importance.)
2. **Importance join**. For each absentee, fetch api-sports player stats (1 call/fixture) and attach `importance_weight` (e.g. minutes-share or normalized rating), joining Flashscore name ↔ api-sports player by fuzzy name + team.
3. **Injury adjuster** (post-model, capped). New `apply_availability_adjustment(probs, home_avail, away_avail)` modelled on `_apply_red_card_modifier`: weight = Σ over absences of `reason_weight × importance_weight`, shift 1X2 away from the more-depleted side toward opponent + draw, magnitude ∝ weight-differential, capped at `MAX_AVAIL_SHIFT`. `reason_weight`: injury/suspension = 1, inactive ≈ 0.2, doubtful ≈ 0.5. O/U effect deliberately omitted in v1 (state-dependent, like the red-card O/U decision). Wire into `predict_matches.py` between Platt calibration and the heuristic adjuster, behind a per-sport `use_availability_adjustment` flag (default off) in `betting_config.json`.
4. **Forward validation + calibration**. Log each match's pre/post-adjustment probs + absence list to a JSONL; once enough settle, compare adjusted vs unadjusted Brier on real outcomes (forward-only — see caveat above). Tune `MAX_AVAIL_SHIFT` + reason/importance weights on that, and gate magnitude *down* in well-covered leagues where the absence is already priced.
...
- **✅ api-sports is the clean importance source.** One `/players?team&season` call per fixture returns every squad player's minutes / appearances / rating / position in a single response; join the Flashscore absentee *names* (+ team + league) to it. Light (1 call/fixture, well under 100/day free) and gives a usable weight directly. **Division of labor: Flashscore = "who's out + why" (free, structured, night-before); api-sports = "how good are they" (importance).**

**THE make-or-break question (gates the whole of D4) — verify before any build:** does api-sports return usable per-player stats (minutes/rating) for our **less-covered** target leagues, not just the top-5? The edge only exists where the market is slow (the long tail). If api-sports player coverage is top-5-only, importance is available exactly where it's useless and absent exactly where we'd need it → D4 is not viable as scoped, and the fallback is Flashscore player-page scraping (heavy) or Transfermarkt value (ToS-hostile). **This single coverage fact is the go/no-go.** Also confirm: name-join feasibility (Flashscore "Wahi E." ↔ api-sports "Elye Wahi" — fuzzy, like the Pamestoixima join) and recent-past queryability for the forward-validation backfill.
