...
| 7c  | **Auto-cashout on ESTIMATED value (functionality test)** | ✅ done (2026-05-26) | Turns the existing display badge (`lock_in` ≥1.5× stake / `stop_loss` adj_prob <0.20) into an automatic action — decoupled from the 8b real-offer data wait by pricing at the synthetic fair-value estimate. `_cashout_decision()` (`web_ui/app.py`) is now the single source of truth for both the badge and the executor; `POST /football/auto_cashout` sweeps every OPEN bet on a live match, fires the lane-cascading `execute_cashout` on non-`hold` decisions, and logs every evaluation (fired or held) to `output/auto_cashout_log.jsonl`. UI: a new **"Auto-cashout" checkbox** on the dashboard live header rides the auto-refresh loop (now **10 min**, relabelled "Auto 10m"); when on, the tick chains `/auto_cashout` after the Flashscore refresh completes and reloads only if something was cashed. Flashscore-only (the auto loop never triggers Pamestoixima). **Virtual money only — no real bet touched.** Tests the cashout TIMING/MECHANISM, not real economics (synthetic value has no real haircut → resulting P/L is optimistic; re-score against real offers once 8b data exists). Thresholds are module constants (`_AUTO_CASHOUT_LOCK_IN_RATIO`, `_AUTO_CASHOUT_STOP_LOSS_PROB`) — tune from the audit log. |
