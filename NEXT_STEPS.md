...
| 7c  | **Auto-cashout on ESTIMATED value (functionality test)** | ✅ done (2026-05-26) | Turns the live cashout decision into an automatic action — decoupled from the 8b real-offer data wait by pricing at the synthetic fair-value estimate. `_cashout_decision()` (`web_ui/app.py`) is the single source of truth for both the display badge and the executor. **Decision (driven by `adj_prob`, which is the LiveAdjuster's synthesis of the live stats — score, minute, xG, shots, possession, dominance, red cards):** `lock_in` if the bet is in profit AND (`adj_prob ≥ 0.85` near-certain — *odds-independent* OR `fair/stake ≥ 1.5` big unrealized profit); `stop_loss` if `adj_prob < 0.20`; else `hold`. A **minute floor (≥30)** suppresses both before in-play stats are reliable. **Why not the old ≥1.5×-stake-only rule:** `fair/stake = odds×adj_prob×0.95` is capped by the odds, so bets at odds <~1.58 could never lock in even at 99% live win prob (e.g. €2 @1.40 maxes at 1.33×) — the probability branch fixes that. `POST /football/auto_cashout` sweeps every OPEN bet on a live match, fires the lane-cascading `execute_cashout` on non-`hold` decisions, and logs every evaluation (fired or held) to `output/auto_cashout_log.jsonl`. UI: a **"Auto-cashout" checkbox** on the dashboard live header rides the auto-refresh loop (now **10 min**, "Auto 10m"); when on, the tick chains `/auto_cashout` after the Flashscore refresh completes and reloads only if something was cashed. Flashscore-only (auto loop never triggers Pamestoixima). **Virtual money only — no real bet touched.** Tests TIMING/MECHANISM, not real economics (synthetic value has no real haircut → P/L optimistic; re-score against real offers once 8b data exists). Thresholds are module constants (`_AUTO_CASHOUT_LOCK_IN_PROB`, `_AUTO_CASHOUT_LOCK_IN_RATIO`, `_AUTO_CASHOUT_STOP_LOSS_PROB`, `_AUTO_CASHOUT_MIN_MINUTE`) — tune from the audit log. |
