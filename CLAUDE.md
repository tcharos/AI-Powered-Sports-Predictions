...
- Manual `Refresh Live Snapshot` POSTs to `/football/refresh_live?with_bookmaker=1` and spawns a parallel `read-open-bets` Popen. The button's JS (`triggerLive`) must fetch the form's own `action` URL so the `?with_bookmaker=1` param survives. Auto-5m stays Flashscore-only (headless Pamestoixima is **Akamai-blocked** — returns "Access Denied"; real-betting step 6d ⛔, see `PAMESTOIXIMA_NOTES.md`).
